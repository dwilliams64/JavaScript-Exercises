...
const convert = (value) => {
    switch (convertTemps.value) {
        case "f-to-c":
            ...
            resultTemp.textContent = `${(value - 32) / 1.8}`;
            break;
        case "c-to-f":
            inputLabel.textContent = "Celsius";
            resultLabel.textContent = "Fahrenheit";            
            resultTemp.textContent = `${(value * 1.8) + 32}`;
            break;
        case "f-to-k":
            inputLabel.textContent = "Fahrenheit";
            resultLabel.textContent = "Kelvin";
            // (T(°F) + 459.67)× 5/9            
            resultTemp.textContent = `${(value + 459.67) * (5/9)}`;
            break;
        case "c-to-k":
            inputLabel.textContent = "Celsius";
            resultLabel.textContent = "Kelvin"; 
            // T(°C) + 273.15                      
            resultTemp.textContent = `${value + 273.15}`;
            break;
        case "k-to-f":
            inputLabel.textContent = "Kelvin";
            resultLabel.textContent = "Fahrenheit";
            // T(K) × 9/5 - 459.67
            resultTemp.textContent = `${(value * (9/5)) - 459.67}`;
            break;
        case "k-to-c":
            inputLabel.textContent = "Kelvin";
            resultLabel.textContent = "Celsius";
